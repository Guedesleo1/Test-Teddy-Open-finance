...
const makeSut = (): SutType => {
  ...
  const sut = new DeleteShortsLinksUseCase({
...
describe('Delete Shrots Links UseCase', () => {
  ...
  it('Should be shortsLinksRepositoryStub.findByUrlShorts, urlShrots not exists', async () => {
    ...
    const response = await sut.delete('A4d3EB');

    expect(response.isSuccess).toBeFalsy();
    expect(response.isFailure).toBeTruthy();
    expect(response.error).toEqual('URL shorts not exists.');
  });
  it('Should be shortsLinksRepositoryStub.delete to be called with correct params', async () => {
    const { shortsLinksRepositoryStub, sut } = makeSut();
    ...
    const shortsLinksRepositorySpy = jest.spyOn(shortsLinksRepositoryStub, 'delete');

    const result = await sut.delete('A4d3EB');
